...
class CiudadController:
    ...
    def post_ciudad(self,data):
        ...
        if not ciudad_nombre or not region_key:
            ...
            nueva_ciudad = self.models.DIM_CIUDAD(ciudad_nombre=ciudad_nombre, region_key=region_key)
            try:
                self.getDb().session.add(nueva_ciudad)
    ...
    def get_ciudad(self):
        ...
        if not all_ciudad.items:
            return {"message": "No hay ciudades registradas."}, 404
        else:
            return {
    ...
    def put_ciudad(self, id, data):
        ...
        if not data.ciudad_nombre:
            return {"message": "El atributo 'ciudad_nombre' es requerido."}, 400
        ...
        try:
            ...
            self.getDb().session.rollback()
            return {"message": f"Error al guardar los cambios: {str(e)}"}, 500
